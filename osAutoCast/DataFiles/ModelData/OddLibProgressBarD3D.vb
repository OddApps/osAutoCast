...
Public Class OddLibProgressBarD3D
    ...
    Public Property BarLayout As BarLayoutMode = BarLayoutMode.FixedAt
    Public Property BarTrackWidth As Single
    Public Property BarTrackHeight As Single
    Public Property BarPadding As Single = 0.0F         ' used by StretchToWindow layout

    ' For BarLayoutMode.FixedAt
    Public Property BarRect As osRect.RawRectangleF = New osRect.RawRectangleF(100, 100, 400, 28)

    ' Optional rounding/appearance
    ...
    Public Property TextColorARGB As osRect.RawColor4 = New osRect.RawColor4(1.0F, 1.0F, 1.0F, 1.0F)

    Protected Overrides Sub OnResize(e As EventArgs)
        MyBase.OnResize(e)
        If _swapChain IsNot Nothing AndAlso ClientSize.Width > 0 AndAlso ClientSize.Height > 0 Then
            ResizeSwapChain(ClientSize.Width, ClientSize.Height)
        End If
    ...
    Public Sub BeginPrep()
        ...
        CreateSizeDependentResources(ClientSize.Width, ClientSize.Height)
    End Sub
    ...
    Private Sub InitDeviceAndSwapChain()
        ...
        Dim mode = New ModeDescription(ClientSize.Width, ClientSize.Height,
        ...
        Using backBuffer As Texture2D = _swapChain.GetBackBuffer(Of Texture2D)(0)
            ...
            Using dxgiSurface As Surface = backBuffer.QueryInterface(Of Surface)()
                Dim props = New RenderTargetProperties(RenderTargetType.Default,
                    New D2DPixelFormat(Format.B8G8R8A8_UNorm, AlphaMode.Ignore),
    ...
    Private Sub InitProgressTimer()
        ...
        AddHandler ProgressTimer.Tick,
         Sub()
             ...
             ' Done?
             Dim tMs = (Stopwatch.GetTimestamp() - _startTicks) * 1000.0 / Stopwatch.Frequency
             If tMs >= _durationMs Then
                 ProgressTimer.Stop()
    ...
    Public Async Function RunProgressAsync(duration As TimeSpan,
                                           Optional easing As Func(Of Double, Double) = Nothing,
                                           Optional label As String = Nothing,
                                           Optional labelMs As Integer = 1500,
        ...
        ProgressEaseFunc = If(easing, AddressOf EaseInOutSine)
        _startTicks = Stopwatch.GetTimestamp()
        ProgressTaskSrc = New TaskCompletionSource(Of Boolean)(TaskCreationOptions.RunContinuationsAsynchronously)
        _cts = CancellationTokenSource.CreateLinkedTokenSource(ct)

        If Not String.IsNullOrWhiteSpace(label) Then
            _textToShow = label
            _textUntilTicks = Stopwatch.GetTimestamp() + CLng((labelMs / 1000.0) * Stopwatch.Frequency)
        Else
            _textToShow = Nothing
            _textUntilTicks = 0
        End If

        ' Render loop on UI thread; Present(1,0) VSyncs → steady pacing, low CPU
    ...
    Private Sub RenderFrame()
        ' Time & eased fraction
        Dim tMs = (Stopwatch.GetTimestamp() - _startTicks) * 1000.0 / Stopwatch.Frequency
        Dim t = Math.Max(0.0, Math.Min(1.0, tMs / _durationMs))
        Dim f = ProgressEaseFunc(t) ' 0..1

        Dim w = Math.Max(1, ClientSize.Width)
        Dim h = Math.Max(1, ClientSize.Height)

        Dim track As osRect.RawRectangleF = New osRect.RawRectangleF(0, 0, BarTrackWidth, BarTrackHeight)

        If Not _primed Then
            PrimeBackground(track)
        End If

        Dim curFillPx As Integer = CInt(Math.Round(f * (track.Right - track.Left)))
        ...
        End If
        ' Delta rectangle is the narrow strip added/removed

        'Dim fillRight As Single = track.Left + CSng(Math.Max(0.0, Math.Min(1.0, f)) * (track.Right - track.Left))
    ...
    Private Sub RenderProgress(progPercent As Double)
        ...
        Dim fillRight As Single = track.Left + CSng(Math.Max(0.0, Math.Min(1.0, (progPercent / 100))) * (track.Right - track.Left))
        Dim fill As New osRect.RawRectangleF(track.Left, track.Top, fillRight, track.Bottom)
    ...
    Public Sub SetProgColor(pColor As osColor.Color, Optional pUpdate As Boolean = False)
        BarColorARGB = New osRect.RawColor4(pColor.R / 255.0F,
                                            pColor.G / 255.0F,
                                            pColor.B / 255.0F,
                                            1.0F)
    End Sub

    Public Sub SetProgSize(pW As Integer, pH As Integer)
        Me.Size = New osDraw.Size(pW, pH)

        SetBarSize(pW, pH)
    End Sub

    Public Sub SetProgPosition(pX As Integer, pY As Integer)
        Me.Location = New osDraw.Point(pX, pY)
    End Sub

    Public Sub SetBarSize(pW As Single, pH As Single)
        BarTrackWidth = Math.Max(1.0F, pW)
        BarTrackHeight = Math.Max(1.0F, pH)
    End Sub

    Public Sub SetBarRect(x As Single, y As Single, width As Single, height As Single)
        BarRect = New osRect.RawRectangleF(x, y, Math.Max(1.0F, width), Math.Max(1.0F, height))
    End Sub

    Public Sub SetWindowRect(x As Integer, y As Integer, w As Integer, h As Integer)
        Me.Bounds = New osDraw.Rectangle(x, y, w, h)
    End Sub

    ' --- Easing options (fast & smooth) ---
