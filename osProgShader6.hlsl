...
float4 osProgShader_Main(osProgShader_Input pin) : SV_Target
{
    ...
    // --- NEW: background-only sentinel ---
    // If currValue is negative (e.g. -1.0f), render only the background.
    // Use `currValue == -1.0f` instead if you prefer an exact match.
    if (currValue < 0.0f)
    {
        // Match the linear pipeline used in the fullCompose branch.
        float3 bg_lin = pow(saturate(pcolorbg.rgb), 2.2f);
        return float4(bg_lin, 1.0f);
    }
