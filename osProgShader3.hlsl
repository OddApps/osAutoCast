...
float4 osProgShader_Main(osProgShader_Input pin) : SV_Target
{
    ...
    if (borderParams.x >= 0.5)
    {
        float thicknessPx = max(0.5, borderParams.y); // at least 0.5 px
        float pxU = thicknessPx * invSize;

        float barH = max(1.0, borderParams.z);
        float v = pin.pos.y / barH;

        float pxV = thicknessPx / barH;

        bool left   = u <= pxU;
        bool right  = u >= (1.0 - pxU);
        bool top    = v <= pxV;
        bool bottom = v >= (1.0 - pxV);

        if (left || right || top || bottom)
        {
            return float4(6.0/255.0, 6.0/255.0, 6.0/255.0, 1.0);
        }
    }
